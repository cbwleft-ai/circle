/**
 * 对话附件落盘与消息富化（多模态/图片输入支持）
 *
 * 当用户在 IM 中发送图片（或文件）时，IM 适配器将附件以
 * `ChatMessage.attachments` 携带进来；`AttachmentStore` 负责把附件
 * 写入数据目录（`{dataDir}/uploads/`），并把本地路径以 `【图片】<path>`
 * 标记注入到发往 Coordinator 的消息文本中，使 Coordinator 能感知图片
 * 并派发任务给 Worker 读取/描述图片（Worker 侧由视觉模型 + read 工具完成看图）。
 */
...
export function buildMessageWithAttachments(
  ...
  if (!trimmed) {
    return `用户发送了以下附件，请处理（图片请读取并描述内容 / 识别图中文字 OCR）：\n${joined}`;
  }
  return `${joined}\n用户消息：${trimmed}`;
}
