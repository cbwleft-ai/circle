...
export async function runUnitTests(): Promise<TestResult[]> {
  ...
  // ---------- 会话隔离（issue #53） ----------
  results.push(
    await runCase("U-50", "会话隔离", "任务数据按会话归属：列表/读取/发送/管理员覆盖", async (t) => {
      const dir = mkdtempSync(join(tmpdir(), "circle-unit-isolation-"));
      try {
        const { AgentTeam } = await import("../src/team/agent-team.js");
        const { loadConfig } = await import("../src/config.js");
        const team = new AgentTeam({
          config: { ...loadConfig(), dataDir: dir, adminChatIds: ["admin"] },
        ...
        try {
          ...
          t.assert(team.getTaskResult("admin", b.id) === "B 结果", "管理员应可读取任意任务");
          t.assert(
            team.listTasks("admin").includes(a.id) && team.listTasks("admin").includes(b.id),
            "管理员应可看全部任务",
          );
        } finally {
          await team.stop();
    ...
    await runCase("U-51", "会话隔离", "定时任务归属：创建/列表/修改/删除按会话鉴权", async (t) => {
      ...
      try {
        ...
        try {
          ...
          t.assert(
            team.updateSchedule("admin", s.id, { name: "管理员修改" })?.name === "管理员修改",
            "管理员应可跨会话修改",
          );
        } finally {
          await team.stop();
  ...
  results.push(
    await runCase("U-52", "消息合并", "MessageMerger keyOf：同会话不同发送者不合并", async (t) => {
      const { MessageMerger } = await import("../src/core/message-merge.js");
      const processed: string[] = [];
      const merger = new MessageMerger(
        ...
        (m) => `${m.chatId}:${m.senderId}`,
      );
      await Promise.all([
        merger.push({ chatId: "g", senderId: "u1", text: "", attachments: [{ kind: "image", data: "A" }] }),
        merger.push({ chatId: "g", senderId: "u2", text: "\u674e\u56db\u7684\u6d88\u606f" }),
        merger.push({ chatId: "g", senderId: "u1", text: "\u5f20\u4e09\u7684\u63cf\u8ff0" }),
      ]);
      t.assert(processed.length === 2, `不同发送者应分 2 批处理，实际 ${processed.length}`);
      t.assert(
        processed.some((x) => x === "u1:\u5f20\u4e09\u7684\u63cf\u8ff0"),
        `张三批次应合并附件与描述：${processed.join(" | ")}`,
      );
      t.assert(processed.some((x) => x === "u2:\u674e\u56db\u7684\u6d88\u606f"), "李四的消息应独立处理");
      const { mergeMessages } = await import("../src/core/message-merge.js");
      const merged = mergeMessages([
        { chatId: "g", chatType: "group", senderId: "u1", senderName: "张三", text: "a" },
        { chatId: "g", chatType: "group", senderId: "u1", senderName: "张三", text: "b" },
      ]);
      t.assert(
        merged.chatType === "group" && merged.senderId === "u1" && merged.senderName === "张三",
        "合并后应保留 chatType/senderId/senderName",
      );
    }),
  );
